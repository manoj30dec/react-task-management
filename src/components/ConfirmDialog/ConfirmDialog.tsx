...
const ConfirmDialog: React.FC<iConfirm>= ({id,user,onClick}) => {
  ...
  <div className="w-full max-w-lg bg-white shadow-lg rounded-lg p-6 relative">
    <svg xmlns="http://www.w3.org/2000/svg" className="w-3.5 cursor-pointer shrink-0 fill-gray-400 hover:fill-red-500 float-right" viewBox="0 0 320.591 320.591"  onClick={() => onClick && onClick(false)}>
    ...
    <div className="my-4 text-center">
      ...
      <h4 className="text-gray-800 text-base font-semibold mt-4">Are you sure you want to delete task of <span className="text-red-700">{user}</span> task id <span className="text-red-700">{id}</span>?</h4>
      <div className="text-center space-x-4 mt-8">
        <button type="button" className="px-4 py-2 rounded-lg text-gray-800 text-sm bg-gray-200 hover:bg-gray-300 active:bg-gray-200"  onClick={() => onClick && onClick(false)}>Cancel</button>
        <button type="button" className="px-4 py-2 rounded-lg text-white text-sm bg-red-600 hover:bg-red-700 active:bg-red-600" 
        onClick={() => onClick && onClick(true)}>Delete</button>
      </div>
    </div>
  </div>
</div>
